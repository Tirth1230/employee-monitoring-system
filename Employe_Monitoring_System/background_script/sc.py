...
def get_device_info():
    return f"{platform.system()}{platform.release()} | {platform.processor()} | Hostname:{socket.gethostname()}"
...
def upload_screenshot_to_django(image_path, username, timestamp):
    ...
    try:
        with open(image_path, 'rb') as img:
            files = {'image': img}
            data = {'username': username, 'timestamp': timestamp, 'data':get_device_info(), 'ip':socket.gethostbyname(socket.gethostname())}
